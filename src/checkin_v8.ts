...
// ------------------------------ live strain --------------------------------
// Strain is the one number that keeps moving through the day, so a brief may
// write the tokens {{STRAIN}}, {{STRAIN_DASH}}, {{STRAIN_KCAL}} and
// {{STRAIN_AT}} instead of fixed numbers. The server fills them from the
// newest Whoop cycle on every page load (index.ts re-syncs in the background),
// and the page keeps refreshing them from /api/live while it is open. An
// archived day shows the last values seen while that day was still current.
...
function readLiveSnapshot(): LiveSnapshot {
	const now = new Date();
	try {
		const row = getDb()
			.prepare('SELECT strain, kilojoule, synced_at FROM cycles ORDER BY start_time DESC LIMIT 1')
			.get() as
			| { strain: number | null; kilojoule: number | null; synced_at: string | null }
			| undefined;
		if (!row) return { strain: null, kcal: null, at: ammanClock(now), stale: true };
		const synced = parseSyncedAt(row.synced_at) ?? now;
		const ageMin = (now.getTime() - synced.getTime()) / 60000;
		return {
			strain: typeof row.strain === 'number' ? row.strain : null,
			kcal: typeof row.kilojoule === 'number' ? Math.round(row.kilojoule / 4.184) : null,
			at: ammanClock(synced),
			stale: ageMin > LIVE_STALE_MIN,
		};
	} catch {
		return { strain: null, kcal: null, at: ammanClock(now), stale: true };
	}
}

function strainDash(strain: number | null): string {
	const pct = strain === null ? 0 : Math.max(0, Math.min(1, strain / 21));
	const on = Math.round(pct * RING_C * 10) / 10;
	const off = Math.round((RING_C - on) * 10) / 10;
	return `${on} ${off}`;
}

function liveNote(snap: LiveSnapshot, isToday: boolean): string {
	if (snap.strain === null) return isToday ? 'not in yet' : 'no reading';
	if (!isToday) return 'final for the day';
	if (snap.stale) return `as of ${snap.at} · waiting on the watch`;
	return `as of ${snap.at} · still counting`;
}

function applyLiveTokens(html: string, snap: LiveSnapshot, isToday: boolean): string {
	if (!html.includes('{{STRAIN')) return html;
	return html
		.split('{{STRAIN_DASH}}')
		.join(strainDash(snap.strain))
		.split('{{STRAIN_KCAL}}')
		.join(snap.kcal === null ? '—' : String(snap.kcal))
		.split('{{STRAIN_AT}}')
		.join(liveNote(snap, isToday))
		.split('{{STRAIN}}')
		.join(snap.strain === null ? '—' : snap.strain.toFixed(1));
}

function saveLiveSnapshot(date: string, snap: LiveSnapshot): void {
	try {
		getDb().prepare('UPDATE briefs SET live = ? WHERE date = ?').run(JSON.stringify(snap), date);
	} catch {
		// the snapshot is a convenience for the archive; never break a page render
	}
}

function readStoredSnapshot(date: string): LiveSnapshot | null {
	try {
		const row = getDb().prepare('SELECT live FROM briefs WHERE date = ?').get(date) as
			| { live: string | null }
			| undefined;
		if (!row || !row.live) return null;
		const parsed = JSON.parse(row.live) as LiveSnapshot;
...
export function registerCheckinRoutes(app: Express): void {
	app.get('/checkin', (req: Request, res: Response) => {
		...
		if (briefHtml.includes('{{STRAIN')) {
			if (isToday) {
				const snap = readLiveSnapshot();
				briefHtml = applyLiveTokens(briefHtml, snap, true);
				saveLiveSnapshot(date, snap);
			} else {
				const frozen = readStoredSnapshot(date) ?? {
					strain: null,
					kcal: null,
					at: '',
					stale: true,
				};
				briefHtml = applyLiveTokens(briefHtml, frozen, false);
			}
		}
	...
	app.get('/api/live', (req: Request, res: Response) => {
		...
		res.json({
			strain: snap.strain,
			text: snap.strain === null ? '\u2014' : snap.strain.toFixed(1),
			dash: strainDash(snap.strain),
			kcal: snap.kcal,
			note: liveNote(snap, true),
			at: snap.at,
		});
	});
...
(function(){
  ...
  // Keep the strain ring moving while the page is open: strain climbs all day,
  // so a number written at the morning rebuild is out of date by lunchtime.
  function paintLive(j){
    if(!j)return;
    var v=document.querySelector('[data-live="s-val"]');
    if(v&&j.text)v.textContent=j.text;
    var a=document.querySelector('[data-live="s-arc"]');
    if(a&&j.dash)a.style.strokeDasharray=j.dash;
    var n=document.querySelector('[data-live="s-note"]');
    if(n&&j.note)n.textContent=j.note;
  }
  function pullLive(){
    if(!document.querySelector('[data-live="s-val"]'))return;
