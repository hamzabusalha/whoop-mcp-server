...
import { registerCheckinRoutes, checkinToolDefs, handleCheckinTool } from './checkin.js';
...
// --- keep the stored Whoop data fresh ---------------------------------------
// Whoop rotates the refresh token on every use, so two refreshes fired at the
// same moment invalidate each other (seen as "Token refresh failed" 400/500 on
// sync_data). syncDays() runs its four fetches in parallel, so a refresh that
// falls due mid-sync is exactly that case. Collapse concurrent refreshes into
// one shared promise.
type RefreshPatch = { refreshTokens: () => Promise<void>; inFlightRefresh?: Promise<void> | null };
const patchable = client as unknown as RefreshPatch;
const originalRefresh = patchable.refreshTokens.bind(client);
patchable.refreshTokens = (): Promise<void> => {
	if (!patchable.inFlightRefresh) {
		patchable.inFlightRefresh = originalRefresh().finally(() => {
			patchable.inFlightRefresh = null;
		});
	}
	return patchable.inFlightRefresh;
};

// The check-in page reads today's strain straight from the database, and strain
// climbs all day, so the database has to keep up on its own rather than waiting
// for the once-a-day morning rebuild.
const AUTO_SYNC_MS = 30 * 60 * 1000;
let autoSyncRunning = false;

async function autoSync(): Promise<void> {
	if (autoSyncRunning || !db.getTokens()) return;
	autoSyncRunning = true;
	try {
		await sync.quickSync();
	} catch {
		// Whoop throws transient 5xx on its auth server; the next tick retries
	} finally {
		autoSyncRunning = false;
	}
}

setTimeout(() => void autoSync(), 60 * 1000);
setInterval(() => void autoSync(), AUTO_SYNC_MS);
	...
	server.setRequestHandler(CallToolRequestSchema, async request => {
		...
		try {
			...
			switch (name) {
				...
				case 'get_brief':
				case 'publish_brief': {
					return handleCheckinTool(name, (args ?? {}) as Record<string, unknown>);
				}
